...
router.get('/points-history', async (req, res) => {
  try {
    ...
    // Log warning if user has lifetimeEsproCoins but no transactions (transactions should be created during Odoo sync)
    if (totalInDB === 0 && req.user.lifetimeEsproCoins > 0) {
      console.warn('[Points History] User has lifetimeEsproCoins but no transactions found. Transactions should be created during Odoo sync:', {
        userId: userObjectId.toString(),
        lifetimeEsproCoins: req.user.lifetimeEsproCoins,
        esproCoins: req.user.esproCoins,
        odooCardId: req.user.odooCardId || 'none',
        message: 'Run Odoo sync to create transaction records',
      });
    }
